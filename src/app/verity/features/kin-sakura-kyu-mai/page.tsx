...
const SLUG = 'kin-sakura-kyu-mai'
const MAIN_CID = 'mird00284'
const ACTRESS_TAGS = ['九野ひなの', '金松季歩', '桜空もも', '伊藤舞雪'] as const

// ── generateMetadata ──────────────────────────────────────────────────────────

export async function generateMetadata() {
  const feature = getFeature(SLUG)!
  const title = `金桜九舞 × MOODYZ25周年 — ${feature.seriesLabel} | VERITY`
  return {
    title,
      ...
      'script:ld+json': JSON.stringify([
        ...
        },
        {
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 3, name: '金桜九舞', item: `${BASE}/verity/features/${SLUG}` },
          ],
        },
      ]),
    },
...
async function getLatestArticleByTag(tag: string): Promise<Article | null> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('articles')
    .select('id, external_id, title, slug, image_url, metadata, published_at, tags')
...
// 編集長指定のピンポイントソロ最新作スラッグ（金桜九舞が重複取得されるのを防ぐ）
const PINNED_SOLO_SLUGS: Partial<Record<string, string>> = {
  '金松季歩': '-snos00269',
  '桜空もも': 'ttmbody--ipzz00850',
  '伊藤舞雪': '--cawd00972',
}

async function countArticlesByTags(tags: readonly string[]): Promise<number> {
  const supabase = await createClient()
  ...
  await Promise.all(
    tags.map(async (tag) => {
      const { data } = await supabase
        ...
        .eq('is_active', true)
        .contains('tags', [tag])
      if (data) data.forEach((r: { id: string }) => idSet.add(r.id))
    }),
  )
  return idSet.size
}
...
function proxiedByCid(cid: string): string {
    ...
    <article className="group relative flex flex-col overflow-hidden rounded-xl border border-[#d4af37]/20 bg-black/40 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#d4af37]/50 hover:shadow-[0_0_28px_rgba(212,175,55,0.18)]">
      {label && (
        <div className="px-3 pt-2.5">
          <span className="text-[9px] font-bold tracking-wider text-[#d4af37]/60 uppercase">{label}</span>
        </div>
      )}
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-[#0d0a00]">
        <ProxiedImage
          src={imgSrc}
          alt={article.title}
          className="absolute inset-0 h-full w-full object-cover object-right transition-transform duration-300 group-hover:scale-[1.05]"
        />
      ...
      <div className="flex flex-col gap-2 p-3">
        {article.slug ? (
          <Link href={`/verity/articles/${article.slug}`}>
            <p className="text-[11px] font-medium leading-snug text-white/85 line-clamp-2 transition-colors group-hover:text-[#d4af37]/90">
              {article.title}
            </p>
          </Link>
        ) : (
          <p className="text-[11px] font-medium leading-snug text-white/85 line-clamp-2">
            {article.title}
          </p>
        )}
...
function MainWorkCard({ article, fanzaUrl }: { article: Article; fanzaUrl: string | null }) {
  const imgSrc = proxiedJacket(article)
  const upcoming = isUpcoming(article.published_at)

  const inner = (
    <article className="group relative flex flex-col sm:flex-row overflow-hidden rounded-2xl border border-[#d4af37]/35 bg-black/50 transition-all duration-200 hover:border-[#d4af37]/60 hover:shadow-[0_0_48px_rgba(212,175,55,0.25)]">
      {/* ジャケット */}
      <div className="relative aspect-[2/3] w-full sm:w-48 shrink-0 overflow-hidden bg-[#0d0a00]">
        <ProxiedImage
          src={imgSrc}
          alt={article.title}
          className="absolute inset-0 h-full w-full object-cover object-right transition-transform duration-300 group-hover:scale-[1.05]"
        ...
        <div className="pointer-events-none absolute inset-0 hidden items-center justify-center bg-black/0 transition-all duration-200 group-hover:bg-black/50 md:flex">
          <span className="translate-y-1 scale-95 rounded-full bg-[#d4af37]/90 px-4 py-1.5 text-[11px] font-bold text-[#0a0800] opacity-0 shadow-lg transition-all duration-200 group-hover:translate-y-0 group-hover:scale-100 group-hover:opacity-100">
            ▶ FANZAで観る
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col justify-between gap-4 p-5">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-[#d4af37]/50 bg-[#d4af37]/10 px-2.5 py-0.5 text-[9px] font-bold tracking-widest uppercase text-[#d4af37]">
            <Crown size={9} />
            MOODYZ 25TH ANNIVERSARY
          </div>
          {article.slug ? (
            <Link href={`/verity/articles/${article.slug}`}>
              <h3 className="text-base font-bold leading-snug text-white/90 transition-colors hover:text-[#d4af37]/90 line-clamp-3">
                {article.title}
              </h3>
            </Link>
          ) : (
            <h3 className="text-base font-bold leading-snug text-white/90 line-clamp-3">
              {article.title}
            </h3>
          )}
          {article.published_at && (
            <p className="text-[10px] text-[#d4af37]/50">{fmtDate(article.published_at)}</p>
          )}
          <div className="flex flex-wrap gap-1.5 pt-1">
            {ACTRESS_TAGS.map((name) => (
              <span
                key={name}
                className="rounded-full border border-[#d4af37]/25 bg-[#d4af37]/8 px-2 py-0.5 text-[9px] font-bold text-[#d4af37]/70"
              >
                {name}
              </span>
            ))}
        ...
        {fanzaUrl && (
          <FanzaLink
            href={fanzaUrl}
            targetId={article.external_id}
            position="spotlight_main_cta"
            className="flex items-center justify-center gap-2 rounded-xl py-3 text-[13px] font-black text-[#0a0800] transition-all hover:brightness-110 hover:shadow-[0_0_24px_rgba(212,175,55,0.5)] bg-gradient-to-r from-[#b8960c] to-[#d4af37]"
          >
            {upcoming ? '今すぐ予約する' : 'FANZAで観る'}
  ...
  return fanzaUrl ? (
    <FanzaLink href={fanzaUrl} targetId={article.external_id} position="spotlight_main_image" className="contents">
      {inner}
    </FanzaLink>
  ) : (
    inner
...
export default async function KinSakuraKyuMaiPage() {
  const feature = getFeature(SLUG)!

  const [mainArticle, actressWorks, totalCount, isOverseas] = await Promise.all([
    getArticleByCid(MAIN_CID),
    Promise.all(
      ACTRESS_TAGS.map((tag) => {
        const pinned = PINNED_SOLO_SLUGS[tag]
        return pinned ? getArticleBySlug(pinned) : getLatestArticleByTag(tag)
      }),
    ),
    countArticlesByTags(ACTRESS_TAGS),
    getIsOverseasUser(),
  ])

  const heroImgSrc = mainArticle
    ? proxiedJacket(mainArticle)
    : proxiedByCid(MAIN_CID)
  ...
  return (
    <div className="min-h-screen bg-[#0a0800]">
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          ヒーローセクション
      ══════════════════════════════════════════════════════════════════════ */}
      <div className="relative min-h-[520px] sm:min-h-[600px] overflow-hidden flex items-end">

        {/* 右側 ambient 背景 — 4女優の最新ソロ作を2×2グリッドで薄くフェード */}
        <div className="pointer-events-none absolute inset-y-0 right-0 hidden sm:grid sm:grid-cols-2 w-[55%] overflow-hidden">
          {actressWorks.map((a, i) => (
            <div key={i} className="relative overflow-hidden">
              {a ? (
                ...
                <div className="absolute inset-0 bg-[#0d0a00]" />
              )}
            </div>
          ))}
          {/* 左フェード */}
          <div
            ...
            style={{
              background:
                'linear-gradient(to right, #0a0800 0%, rgba(10,8,0,0.75) 25%, rgba(10,8,0,0.25) 75%, transparent 100%)',
            }}
          />
        </div>
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(to top, #0a0800 22%, rgba(10,8,0,0.72) 55%, rgba(10,8,0,0.35) 100%)',
          }}
        />
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 65% 55% at 25% 50%, rgba(212,175,55,0.14) 0%, transparent 70%)',
          }}
        />
        ...
        <div className="relative mx-auto w-full max-w-5xl px-4 py-14 sm:py-20">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:gap-10">
            ...
            <div className="hidden sm:block shrink-0">
              <div className="relative aspect-[2/3] w-40 overflow-hidden rounded-2xl border border-[#d4af37]/40 shadow-[0_0_56px_rgba(212,175,55,0.28)]">
                <ProxiedImage
                  src={heroImgSrc}
                  alt="金桜九舞"
            ...
            <div className="space-y-4">
              ...
              <nav className="flex items-center gap-1 text-[10px] text-[#d4af37]/40">
                ...
                <ChevronRight size={10} />
                <span className="text-[#d4af37]/60">金桜九舞</span>
              </nav>
              ...
              <div className="inline-flex items-center gap-1.5 rounded-full border border-[#d4af37]/40 bg-[#d4af37]/10 px-3 py-1 text-[10px] font-bold tracking-widest uppercase text-[#d4af37]">
                <Crown size={10} />
                MOODYZ 25th Anniversary × {feature.seriesLabel}
              </div>

              {/* タイトル */}
              <h1
                className="text-4xl sm:text-6xl font-black tracking-tight text-[#d4af37]"
                style={{ textShadow: '0 0 64px rgba(212,175,55,0.6)' }}
              >
                金桜九舞
              </h1>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ACTRESS_TAGS.map((name) => (
                  <span
                    key={name}
                    className="rounded-full border border-[#d4af37]/35 bg-[#d4af37]/8 px-2.5 py-0.5 text-[10px] font-bold text-[#d4af37]/80"
                  >
                    {name}
                  </span>
                ))}
              ...
              <div className="flex items-center gap-3 pt-2">
                <a
                  href="#featured-works"
                  className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#b8960c] to-[#d4af37] px-5 py-2.5 text-sm font-bold text-[#0a0800] transition-all hover:brightness-110 hover:shadow-[0_0_28px_rgba(212,175,55,0.55)]"
                >
                  <Star size={14} />
                  作品を見る
                </a>
                <a
                  href="#sample-movies"
                  className="inline-flex items-center gap-2 rounded-full border border-[#d4af37]/40 bg-[#d4af37]/8 px-5 py-2.5 text-sm font-bold text-[#d4af37] transition-all hover:border-[#d4af37]/70 hover:bg-[#d4af37]/15"
                >
                  <Play size={14} />
                  予告を観る
                </a>
      ...
      <div className="mx-auto max-w-5xl space-y-16 px-4 py-14">
        ...
        <section className="space-y-6">
          <SectionHeader icon={<Sparkles size={18} />} label="4人の魅力" />
            ...
            {feature.whyNotable.map((item, i) => (
              ...
              >
                ...
                <div className="space-y-1.5">
                  <p className="font-black text-[#d4af37] text-[14px]">{item.title}</p>
        ...
        {/* ── セクション2: 数字で見る ── */}
        <section className="space-y-6">
          <SectionHeader icon={<BarChart3 size={18} />} label="数字で見る金桜九舞" />
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard label="総掲載作品数" value={String(totalCount)} unit="件" highlight />
        ...
        <section id="featured-works" className="space-y-6">
          <SectionHeader icon={<Star size={18} />} label="注目作品" />
          ...
          {mainArticle && (
            <div className="mb-8">
              <MainWorkCard
                article={mainArticle}
                fanzaUrl={affiliateUrl(mainArticle, isOverseas)}
              />
            </div>
          )}

          {/* 4女優 最新ソロ作 */}
          {actressWorks.some(Boolean) && (
            <>
              <div className="flex items-center gap-3">
                <div
                ...
                />
                <p className="text-[10px] font-bold tracking-widest text-[#d4af37]/50 uppercase">
                  各女優の最新ソロ作
                </p>
                <div
                  className="h-px flex-1"
                  style={{ background: 'linear-gradient(to left, rgba(212,175,55,0.3), transparent)' }}
                />
              </div>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {actressWorks.map((article, i) =>
                  article ? (
                    <WorkCard
                      key={article.id}
                      article={article}
                      fanzaUrl={affiliateUrl(article, isOverseas)}
                      label={ACTRESS_TAGS[i]}
                    />
                  ) : (
                    <div
                      key={i}
                      className="flex aspect-[2/3] flex-col items-center justify-center rounded-xl border border-[#d4af37]/10 bg-black/20 text-center p-3"
                    >
                      <p className="text-[10px] font-bold text-[#d4af37]/50">{ACTRESS_TAGS[i]}</p>
                      <p className="mt-1 text-[9px] text-white/20">作品取得中</p>
                    </div>
                  ),
                )}
              </div>
        ...
        <section id="sample-movies" className="space-y-6">
          ...
          <div className="overflow-hidden rounded-2xl border-2 border-[#c5a059]/20 bg-black/40 p-3 shadow-[0_0_48px_rgba(197,160,89,0.18),0_0_96px_rgba(197,160,89,0.08)] backdrop-blur-sm">
            {/* ゴールドグロー装飾 */}
            <div
            ...
            <div className="relative w-full overflow-hidden rounded-xl" style={{ paddingTop: '56.25%' }}>
              <iframe
                src={`https://www.dmm.co.jp/litevideo/-/part/=/cid=${MAIN_CID}/size=720_480/affi_id=mizutamari48-990/`}
                className="absolute inset-0 h-full w-full border-0"
                allowFullScreen
                title="金桜九舞 最高峰な妻たち。令和の専属ドリーム大共演 — 予告映像"
                loading="lazy"
              />
            </div>
            <p className="mt-2.5 px-1 text-center text-[11px] leading-snug text-white/45">
              金桜九舞 最高峰な妻たち。令和の専属ドリーム大共演 — 公式予告映像
            </p>
          </div>
                ...
                <div key={i} className="relative pb-8 last:pb-0">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[13px] font-bold ${item.isHighlight ? 'text-[#d4af37]' : 'text-white/75'}`}>
                        {item.label}
                      </span>
                    </div>
                    {item.note && (
        ...
        <section className="space-y-5">
          ...
          {/* 4女優 個別ページ */}
          <div className="space-y-2">
            <p className="text-[9px] font-bold tracking-widest text-[#d4af37]/35 uppercase">出演女優 個別ページ</p>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <RelatedLink href="/verity/actresses/dmm-actress-1078407" label="九野ひなの" sub="女優個別ページ" />
              <RelatedLink href="/verity/actresses/dmm-actress-1092091" label="金松季歩" sub="女優個別ページ" />
              <RelatedLink href="/verity/actresses/dmm-actress-1039157" label="桜空もも" sub="女優個別ページ" />
              <RelatedLink href="/verity/actresses/dmm-actress-1042907" label="伊藤舞雪" sub="女優個別ページ" />
            </div>
          </div>
...
function StatCard({ label, value, unit, highlight }: { label: string; value: string; unit?: string; highlight?: boolean }) {
  return (
    <div
      className={`flex flex-col items-center justify-center gap-1.5 rounded-xl border p-4 text-center backdrop-blur-sm ${
        highlight
          ? 'border-[#d4af37]/40 bg-[#d4af37]/8 shadow-[0_0_20px_rgba(212,175,55,0.12)]'
          : 'border-[#d4af37]/15 bg-black/35'
      }`}
    >
      <p className="text-[9px] font-bold tracking-wider text-[#d4af37]/50 uppercase">{label}</p>
      <p
        className={`font-black text-[#d4af37] ${highlight ? 'text-2xl' : 'text-xl'}`}
