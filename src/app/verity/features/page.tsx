...
export default async function FeaturesIndexPage() {
  ...
  // 各特集の最新記事 + 各 Spotlight のヒーローを並行取得（ヒーロー画像用）
  const [articles, spotlightHeroes] = await Promise.all([
      ...
      <div className="mx-auto max-w-5xl px-4 py-12">
        {(
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {/* ── Spotlight 特集（特設ルート /spotlight/*）— レジストリ駆動 ── */}
            {spotlights.map((s, i) => {
              const hero = spotlightHeroes[i]
              return (
                <SpotlightLink
                  key={s.slug}
                  href={s.href}
                  slug={s.slug}
                  placement="features_index"
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-3 p-4">
                    <p className="line-clamp-2 text-[12px] leading-relaxed text-white/60">{s.tagline}</p>
                    <div className="mt-auto flex items-center justify-between">
                      <span className="text-[10px] text-[#d4af37]/40">{fmtDate(s.publishedAt)}</span>
