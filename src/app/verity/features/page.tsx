...
export default async function FeaturesIndexPage() {
  ...
  // 各特集の最新記事 + メンズエステ特集ヒーローを並行取得（ヒーロー画像用）
  const [articles, mensEstheHero] = await Promise.all([
    Promise.all(features.map((f) => getLatestArticle(f.actressTag))),
      ...
      <div className="mx-auto max-w-5xl px-4 py-12">
        {(
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {/* ── メンズエステ特集（ジャンル特集・特設ルート /spotlight/mens-esthe） ── */}
            <Link
            ...
            >
              <div className="relative aspect-[16/9] overflow-hidden bg-[#0d0a00]">
                ...
                <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full border border-[#d4af37]/50 bg-[#0a0800]/80 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-widest text-[#d4af37] backdrop-blur-sm">
                  <Sparkles size={8} />
                  {MENS_ESTHE_META.seriesLabel}
                </span>
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <p className="text-xl font-black text-[#d4af37]" style={{ textShadow: '0 0 24px rgba(212,175,55,0.6)' }}>
                    {MENS_ESTHE_META.title}
                  </p>
                </div>
              </div>
              <div className="flex flex-1 flex-col gap-3 p-4">
                <p className="line-clamp-2 text-[12px] leading-relaxed text-white/60">{MENS_ESTHE_META.tagline}</p>
                <div className="mt-auto flex items-center justify-between">
                  <span className="text-[10px] text-[#d4af37]/40">{fmtDate(MENS_ESTHE_META.publishedAt)}</span>
